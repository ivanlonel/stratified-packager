...
- **Live virtual layers** (`materialize_virtual_layer=false` with every queried source packaged,
  ...
  rebuilds the definition eagerly — so a `lazy` definition is an authoring convenience that never
  reaches a package. A virtual layer is dropped (with a warning) for any
